...
class UserController extends MobileController
{
	...
	public function getmoneydydx()
	{
		$uid = userid();
		$wallinfo = M("user_coin")->where(array('userid' => $uid))->find();

		$coinapi = "https://api.huobi.pro/market/history/kline?period=1day&size=1&symbol=dydxusdt";
		$result = $this->get_maket_api($coinapi);
		$price_arr  = $result['data'][0];
		$usdt_price = $price_arr['close'];

		$re['num'] = round($wallinfo['dydx'], 5);
		$re['numd'] = round($wallinfo['dydx'], 5);
		$re['zhe'] = round($wallinfo['dydx'] * $usdt_price + $wallinfo['dydxd'] * $usdt_price, 5);
		$re['code'] = 1;

		session("dydxzh", $re['zhe']);
		$this->ajaxReturn($re);
	}
	...
	public function tbhandle()
	{
		if ($_POST) {
			...
			if ($paypassword == '' || $paypassword == null) {
				$this->ajaxReturn(['code' => 0, 'info' => L('Vui lòng nhập mật khẩu rút tiền')]);
			}
			...
			$num = trim(I('post.num'));
			if ($num <= 0) {
				$this->ajaxReturn(['code' => 0, 'info' => L('Vui lòng nhập đúng số tiền')]);
			}
			$cinfo = M("coin")->where(array('id' => $id))->find();
			if (empty($cinfo)) {
				$this->ajaxReturn(['code' => 0, 'info' => L('Lỗi tham số')]);
			}
			if ($num < $cinfo['txminnum']) {
				$this->ajaxReturn(['code' => 0, 'info' => L('Nó không thể thấp hơn giá trị tiền tệ tối thiểu')]);
			}
			if ($num > $cinfo['txmaxnum']) {
				$this->ajaxReturn(['code' => 0, 'info' => L('Nó không thể cao hơn giá trị tiền tệ tối đa')]);
			}
	...
	public function upplusaddress()
	{
		if ($_POST) {
			...
			$abank = trim(I('post.abank'));
			$usdtaddr = trim(I('post.usdtaddr'));
			$czline = trim(I('post.czline'));
			if ($oid <= 0) {
	...
	public function txpage()
	{
		...
		$adrinfo = M("user_qianbao")->where(array('userid' => $uid))->order('id desc')->limit(1)->find();
		if (!empty($adrinfo)) {
